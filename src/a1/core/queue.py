...
class EventQueue:
    ...
    def __init__(self, max_size: int = 10000, overflow_policy: str = "drop_oldest"):
        ...
        self._metrics = {"events_received": 0, "events_processed": 0, "events_dropped": 0, "queue_overflows": 0}
    ...
    async def get(self, timeout: float | None = None) -> Event | None:
        ...
        try:
            ...
            if not self._priority_queue.empty():
                _, _, event = await asyncio.wait_for(self._priority_queue.get(), timeout=timeout)
    ...
    def get_metrics(self) -> dict[str, int]:
        ...
        Returns:
            Metrics dictionary
        """
        return {**self._metrics, "queue_size": self._queue.qsize(), "priority_queue_size": self._priority_queue.qsize()}
...
class IPCEventTransport:
    ...
    async def start_server(self, handler: Callable[[Event], None]):
        ...
        self.server = await asyncio.start_unix_server(
            lambda r, w: self._handle_client(r, w, handler), path=str(self.socket_path)
        )
    ...
    def _deserialize_event(self, data: dict[str, Any]) -> Event:
        ...
        if event_type == "ClaudeEvent":
            return ClaudeEvent(type=data["type"], data=data["data"], timestamp=timestamp, source=data["source"])
        elif event_type == "QuaestorEvent":
            return QuaestorEvent(type=data["type"], data=data["data"], timestamp=timestamp, source=data["source"])
        else:
            event = Event()
