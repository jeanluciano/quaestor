...
class A1Launcher:
    ...
    def _get_default_config(self) -> dict:
        ...
        return {
            ...
            "hooks": {"forward_to_a1": True, "receive_from_a1": True},
    ...
    async def _start_background_service(self) -> bool:
        ...
        try:
            ...
            # Prepare command
            cmd = [sys.executable, "-m", "a1.service.main", "--mode", "background", "--config", str(self.config_path)]
...
def add_a1_to_cli(app):
    ...
    try:
        ...
        if config.get("enabled", False):
            app.add_typer(a1_app, name="a1", help="A1 automatic intelligence features", hidden=False)
            logger.info("A1 CLI commands added")
        else:
            # Add hidden placeholder
            app.add_typer(a1_app, name="a1", help="A1 automatic intelligence (disabled)", hidden=True)
