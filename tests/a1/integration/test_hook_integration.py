...
class TestHookIntegration:
    ...
    def test_all_hook_handlers(self):
        ...
        for handler_name, expected_type in handlers:
...
class TestHookScript:
    ...
    def test_main_invalid_json(self, capsys):
        ...
        with patch.object(sys, 'argv', ['claude_receiver.py', 'test_hook']):
            with patch('sys.stdin', MockStdin("invalid json")):
                with pytest.raises(SystemExit) as exc_info:
                    from a1.hooks.claude_receiver import main
                    main()
    ...
    def test_main_unknown_hook(self, capsys):
        ...
        with patch.object(sys, 'argv', ['claude_receiver.py', 'unknown_hook']):
            with patch('sys.stdin', MockStdin('{"test": "data"}')):
                with pytest.raises(SystemExit) as exc_info:
                    from a1.hooks.claude_receiver import main
                    main()
    ...
    def test_main_successful_processing(self, capsys):
        ...
        with patch.object(sys, 'argv', ['claude_receiver.py', 'post_tool_use']):
            with patch('sys.stdin', MockStdin('{"tool": "Read", "file": "test.py"}')):
                # Mock the receiver
                with patch('a1.hooks.claude_receiver.ClaudeHookReceiver') as MockReceiver:
                    mock_instance = Mock()
                    mock_instance.handle_post_tool_use.return_value = {
                        "status": "received",
                        "a1": "processing"
                    }
